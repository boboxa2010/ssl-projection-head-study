...
2. SL experiment: Resnet classic pretrain with projection fixed reweightening head on Cifar100 on super classes prediciton with kappa = 1.5:

``` model = ProjectedClassifier(n_classes = 20, encoder = 'ResNet32', mode = 'fixed', kappa = 1.5) ```
``` criterion = CrossEntropyLoss() ```
...
6. SSL experiment: Resnet pretrain on Mnist-on-Cifar10 with SimCLR loss and fixed reweightening proj head with kappa = 1.5:
